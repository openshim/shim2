...
public class ShimObjectTableViewer extends TableViewer {
	...
	public static final Predicate<EStructuralFeature> shouldHideFeature = x -> (
		x.getFeatureID() == x.getEContainingClass().getEIDAttribute().getFeatureID()
	);
		...
		shimObjectTableViewer.setCellModifier(new ICellModifier() {
			...
			public Object getValue(Object element, String property) {
				...
				if (format != null) {
					...
					case OBJECT:
						return ShimUtils.getName((EObject) val);
			...
			public void modify(Object element, String property, Object value) {
				...
				if (ShimUtils.getNameAttribute(so.getObj()).isPresent()) {
					@SuppressWarnings("unchecked")
					List<ShimObject> input = (List<ShimObject>) shimObjectTableViewer
							.getInput();
					List<EObject> objectList = input.stream()
							.map(x -> x.getObj())
				...
				if (format.getType() == ColumnType.COMBO) {
					EList<EEnumLiteral> _enum = ((EEnum) format.getEType()).getELiterals();
					Integer intValue = (Integer) value;
					if (intValue < _enum.size()) {
						valStr = _enum.get(intValue).getLiteral();
					} else{
						valStr = "";
					}
				} else {
				...
				if (CommonConstants.FIELD_NAME.equals(property)) {
					String result = nameAttributeChecker.checkNameList(so.getObj(), valStr);
					if (result != null) {
				...
				if (format != null) {
					...
					case COMBO:
						...
						Integer intValue = (Integer) value;
						if (intValue < _enum.size()) {
							so.setValue(propertyName, _enum.get(intValue));
						} else {
							so.setValue(propertyName, null);
						}
	 ...
	 */
	public void initTabeleViewer(EClass eClass, boolean hasDefinedRegion) {
		final int COLUMN_WIDTH = 150;
		final int COLUMN_WIDTH_FOR_XPATH = 300;

		deleteOldColums();

		this.columnsList = createColumsList(eClass, hasDefinedRegion);
		...
		setLabelProvider(new ShimObjectTableLabelProvider(eClass, columnsList));
		setCellEditors();
	 ...
	 */
	private List<ShimObjectColumnFormat> createColumsList(EClass eClass,
		...
		for (EStructuralFeature f : eClass.getEAllStructuralFeatures()) {
			...
			if (f.getUpperBound() == 1) {
				// It isn't Collection.
				if (ShimObjectTableViewer.shouldHideFeature.test(f)) {
					continue;
				}
				...
				if (type.getEPackage() instanceof ShimPackage) {
					if (type instanceof EEnum) {
						...
						ShimObjectColumnFormat format = new ShimObjectColumnFormat(
								name, ColumnType.OBJECT, type, isRequied);
						columnsList.add(format);
					}
				} else {
					...
					if (insName.equals(CLASS_NAME_STRING)) {
						...
						format = new ShimObjectColumnFormat(name,
							...
							.equals(ReflectionUtils.CLASS_NAME_LONG_WRAPPER)
							|| insName
						...
						format = new ShimObjectColumnFormat(name,
							...
							.equals(ReflectionUtils.CLASS_NAME_OBJECT)) {
						format = new ShimObjectColumnFormat(name,
	...
	private void setCellEditors() {
		...
		for (ShimObjectColumnFormat format : columnsList) {
			...
			case COMBO:
				...
				String[] strvaluesArray = getEnumStrings((EEnum) format.getEType(), isRequired);
	 ...
	 */
	private static String[] getEnumStrings(EEnum eEnum, boolean isRequired) {
		List<String> valueStrings = new ArrayList<String>();

		for (Enumerator cts : eEnum.getELiterals()) {
			String str = cts.getLiteral();
			valueStrings.add(str);
		}
		if (!isRequired) {
			valueStrings.add(ShimObjectTableViewer.NONE);
		}
		...
		protected void doSetValue(Object value) {
			if (value == null) {
				...
				for (int i = 0; i < items.length; i++) {
					if (ShimObjectTableViewer.NONE.equals(items[i])) {
						index = i;
						break;
					}
				}
